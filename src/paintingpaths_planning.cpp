...
int main(int argc,char**argv)
{
    ...
    ros::Publisher octomap_pub = nh.advertise<octomap_msgs::Octomap>(argv[3], 1, false);
    ...
    for(int i=0; i<candidate_waypoints_num; i++)
    {
        ...
        Eigen::Vector3f deltHeight = major_vector * (iHeight*0.24);
        Eigen::Vector3f deltaWidth = middle_vector * (iWidth*0.05);
        Eigen::Vector3f DistanceToWall = -minor_vector * 0.35;
        Eigen::Vector3f deltaMove = deltaWidth + deltHeight + DistanceToWall + Start_Position;
        
        candidate_waypoint_positions[i][0] = deltaMove[0];
        candidate_waypoint_positions[i][1] = deltaMove[1];
        candidate_waypoint_positions[i][2] = deltaMove[2];
        candidate_waypoint_positions[i][3] = direction_eulerAngle[2];
        candidate_waypoint_positions[i][4] = direction_eulerAngle[1];
        candidate_waypoint_positions[i][5] = direction_eulerAngle[0];
        candidate_waypoint_positions[i][6] = direction_eulerAngle[2];
        candidate_waypoint_positions[i][7] = direction_eulerAngle[1];
        candidate_waypoint_positions[i][8] = direction_eulerAngle[0]+M_PI;
